...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <expressions> expression_list  parameter_list
    %type <expression> expression let_stmt chained_let 
    ...
    */
    ...
    feature : OBJECTID ':' TYPEID %prec DARROW{
                    $$ = attr($1, $3, no_expr());
                }
    ...
    expression : OBJECTID ASSIGN expression 
            ...
            | expression '.' OBJECTID '(' parameter_list ')'
                {
                    /*invocation*/
                    $$ = dispatch($1, $3, $5);
                }
            | expression '@' TYPEID '.' OBJECTID '(' parameter_list ')'
                {
                    $$ = static_dispatch($1, $3, $5, $7); 
                }
            | OBJECTID '(' parameter_list ')'  
                {
    ...
    case_list : OBJECTID ':' TYPEID DARROW expression {
            ...
            | case_list ';' OBJECTID ':' TYPEID DARROW expression {
                    $$ = append_Cases($1, single_Cases(branch($3, $5, $7)));
                }
            ;
    ...
    parameter_list : /*empty*/ { $$ = nil_Expressions(); }
            | expression %prec DARROW { $$ =  single_Expressions($1);}
            | parameter_list ',' expression { $$ = append_Expressions($1, single_Expressions($3)); }
            ;

    let_stmt : LET OBJECTID ':' TYPEID IN expression
                {
                    $$ = let($2, $4, no_expr(), $6);
                }
            ...
            | LET OBJECTID ':' TYPEID ',' chained_let 
                {
                    $$ = let($2, $4, no_expr(), $6);
                }
            | LET OBJECTID ':' TYPEID ASSIGN expression ',' chained_let
                {
                    $$ = let($2, $4, $6, $8);
                }
            | LET OBJECTID ':' TYPEID ASSIGN error ',' chained_let
                {
                    $$ = let($2, $4, no_expr(), $8);
                }
            ;

    chained_let : OBJECTID ':' TYPEID ',' chained_let { 
                    $$ = let($1, $3, no_expr(), $5);
                }
            | OBJECTID ':' TYPEID ASSIGN expression ',' chained_let { 
                    $$ = let($1, $3, $5, $7);
                }
